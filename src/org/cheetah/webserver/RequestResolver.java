...
public class RequestResolver {
        ...
        try {
            ...
            if (Files.exists(targetPath)) {
                ...
                if (Files.isRegularFile(targetPath, LinkOption.NOFOLLOW_LINKS)) {
                    ...
                    readFile(Paths.get(targetPath.toString()).toUri().toURL());
                    ...
                    if (this.webserver.isFileFolderBrowsingEnabled()) {
                        ...
                        while (defaultPageTokenizer.hasMoreTokens()) {
                            ...
                            if (Files.exists(indexPath)) {
                                ...
                                readFile(indexPath.toUri().toURL());
                                found = true;
                                break;
    ...
    private void readFile(URL url) throws IOException, FileNotFoundException, URISyntaxException {
        ...
        if (extentionIndex > 0) {
            ...
            if (MimeType.isText(extention)) {
                ...
                Utils.readTextFile(body, url, Charset.forName("utf-8"));

            } else {
                response.setValue("Content-Type", mimeType);
                Utils.readBinaryFile(body, url);
            }
        } else {
            response.setValue("Content-Type", mimeType);
            Utils.readBinaryFile(body, url);
